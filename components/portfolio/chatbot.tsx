...
export function Chatbot() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && eggsFound.size > 0) {
      localStorage.setItem(
        "portfolio-eggs-found",
        JSON.stringify([...eggsFound])
      );
    }
  }, [eggsFound]);
  ...
  // Listen for easter egg discoveries from other components
  useEffect(() => {
    const handleEggFound = (e: CustomEvent<{ eggId: number; eggName: string }>) => {
      const { eggId, eggName } = e.detail;
      if (!eggsFound.has(eggId)) {
        setEggsFound((prev) => {
          const next = new Set([...prev, eggId]);
          const remaining = TOTAL_EGGS - next.size;
          const newMessage: Message = {
            id: Date.now(),
            text:
              remaining > 0
                ? `Amazing! You found "${eggName}"! ${remaining} more to go! Keep hunting!`
                : `WOW! You found "${eggName}"! That's all ${TOTAL_EGGS}! You're a Master Explorer!`,
            isUser: false,
            timestamp: new Date(),
          };
    ...
    window.addEventListener("easterEggFound" as never, handleEggFound as never);
    return () =>
      window.removeEventListener("easterEggFound" as never, handleEggFound as never);
  }, [eggsFound]);
  ...
  const getBotResponse = useCallback(
    (userMessage: string): string => {
      ...
      if (conversationState.awaitingHintConfirmation) {
        if (
          lowerMessage.match(
            /^(yes|yeah|yep|sure|ok|okay|please|yea|y|definitely|absolutely)$/
          )
        ) {
      ...
      if (lowerMessage.match(/^help$|how do i|what can/)) {
        return `Find ${TOTAL_EGGS} hidden surprises in this portfolio! Want a hint? Just ask! You can also ask me about Ananya's projects, skills, experience, or publications.`;
      }
      ...
      if (lowerMessage.match(/easter|eggs?|hidden|secret|hunt|surprise/)) {
        ...
        setConversationState({
          awaitingHintConfirmation: true,
          lastTopic: "easter",
        });
        return `There are ${TOTAL_EGGS} hidden gems in this portfolio! You've found ${found}/${TOTAL_EGGS} so far. Want a hint?`;
      }
      ...
      if (lowerMessage.match(/progress|how many|found|status|score/)) {
        const found = eggsFound.size;
        if (found === 0) {
          return `You haven't found any easter eggs yet! There are ${TOTAL_EGGS} hidden around the portfolio. Want a hint to get started?`;
        }
        if (found === TOTAL_EGGS) {
          return `You found ALL ${TOTAL_EGGS} easter eggs! Master Explorer status achieved! What else can I help you with?`;
        }
        const foundNames = EASTER_EGGS.filter((e) => eggsFound.has(e.id))
          .map((e) => e.name)
          .join(", ");
        return `You've found ${found}/${TOTAL_EGGS} easter eggs! (${foundNames}) Keep exploring!`;
      }
      ...
      if (
        lowerMessage.match(
          /ar|vr|xr|mixed reality|augmented|virtual|metaverse|lens/
        )
      ) {
      ...
      const defaultResponses = [
        ...
        `Good question! I specialize in Ananya's portfolio though. Ask about her skills, projects, or try to find the ${TOTAL_EGGS} hidden secrets!`,
      ];
  ...
  const handleQuickAction = (action: string) => {
    ...
    switch (action) {
      ...
      case "progress": {
        const found = eggsFound.size;
        if (found === 0) {
          responseText = `You haven't found any easter eggs yet! There are ${TOTAL_EGGS} hidden around the portfolio. Want a hint?`;
        } else if (found === TOTAL_EGGS) {
          responseText = `You found ALL ${TOTAL_EGGS} easter eggs! Master Explorer!`;
        } else {
          const foundNames = EASTER_EGGS.filter((e) => eggsFound.has(e.id))
            .map((e) => e.name)
            .join(", ");
          responseText = `You've found ${found}/${TOTAL_EGGS}! (${foundNames})`;
        }
        break;
      }
      default:
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              ...
              {eggsFoundCount > 0 && eggsFoundCount < TOTAL_EGGS && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-accent text-[10px] font-bold rounded-full flex items-center justify-center text-accent-foreground">
                  {eggsFoundCount}
                </span>
              )}
            </motion.div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <div className="px-3 py-1 rounded-full bg-primary/20 border border-primary/30 text-xs text-primary-foreground flex items-center gap-1">
                    ...
                    <span>
                      Found: {eggsFoundCount}/{TOTAL_EGGS}
                    </span>
                  </div>
                  {/* Mini progress bar */}
                  <div className="w-20 h-1.5 bg-secondary/50 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(eggsFoundCount / TOTAL_EGGS) * 100}%`,
                      }}
